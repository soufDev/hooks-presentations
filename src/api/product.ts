import axios, { Canceler } from 'axios';
import { Props as ProductProps } from '../components/Products/Product';
import { Props as DetailProps } from '../components/Products/Details';

const URL = 'https://s3-eu-west-1.amazonaws.com/developer-application-test/cart';

const { CancelToken } = axios;
let cancel: Canceler;
let source = CancelToken.source();

export const fetchProductList = (path: string): Promise<ProductProps[]> => {
  return axios.get(`${URL}${path}`, {
    cancelToken: new CancelToken(function executor(cancellation) { cancel = cancellation })
  }).then(results => results.data.products);
};

export const fetchProductListAbort = () => cancel('cancel product List');

export const fetchProductDetails = (id: string): Promise<DetailProps> => {
  return axios.get(`${URL}/${id}/detail`, {
    cancelToken: new CancelToken(function executor(cancellation){ cancel = cancellation })
  }).then(result => result.data);
};

export const fetchProductDetailsAbort = () => cancel('cancel detail product');
